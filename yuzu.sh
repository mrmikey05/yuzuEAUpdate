    ...
    if [ ! -e "${emuDontUpdate}" ]; then
        ...
        if : >/dev/tcp/8.8.8.8/53; then
            ...
            if [ "$isMainline" = true ]; then
                ...
                # MODIFIED: Will be downloading from pineappleEA for the latest update. Removed all token and BEARER as we don't need it.
                if [ "$emuExeFile" = "$YuzuEA_emuPath" ]; then
					          yuzuEaHost="https://api.github.com/repos/pineappleEA/pineapple-src/releases/latest" #pineappleEA latest api
                    yuzuEaMetadata=$(curl -fSs ${yuzuEaHost})
                    fileToDownload=$(echo ${yuzuEaMetadata} | jq -r '.assets[] | select(.name|test(".*.AppImage$")).browser_download_url')
					          currentVer=$(echo ${yuzuEaMetadata} | jq -r '.tag_name')
